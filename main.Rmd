...
wants <- c("psych","reshape","reshape2","car","carData","moments","tidyverse","stringr","plotly","readxl","ggthemes","GGally","grid","gridExtra","data.table","esquisse","Hmisc","psychTools", "stargazer","polycor","jtools","lavaan","lavaanPlot","corrplot","ggpubr")
...
# set column name to lowerstring
data = setnames(data, tolower(names(data)))
data$pourcentage = as.numeric(data$pourcentage)
# Create all the needed dataframes
data_cleaned_scale = data %>% drop_na(c(7:42)) %>% filter(!(situation=="Sans emploi"))
data_cleaned_wcst = data %>% drop_na(c(7:48)) %>% filter(!(situation=="Sans emploi"))
nflex = data_cleaned_scale[,c(21:30)]
nflex_inv = nflex

# Invert item that are inverted in the scales
...
# Initiate some variables needed for analysis
## Get number of observation
n_row = nrow(data_cleaned_scale)
rm(a, nflex, inverted_item_likert4, inverted_item_likert7, x)
```
...
## For situation/age
ggplot(data=data_ggplot, aes(x=age, fill=situation))+
  geom_histogram(color="#e9ecef", alpha=.9, position ="identity",binwidth = 2) + 
  scale_fill_manual(values=c("#404080", "#b8c2cc", "#69b3a2")) +
  theme_apa() +
  theme(legend.position = "right") +
...
ggplot_data = ggplot_data %>% 
  rename(Q1 = nflex_1,
         ...
         Q9 = nflex_9,
         Q10 = nflex_10)
ggplot(gather(ggplot_data), aes(x=value)) + 
...
ggplot(gather(data_cleaned_scale[,11:20]), aes(x=value)) + 
  geom_histogram(stat = "count") + facet_wrap(~key, scales = 'free_x') +
  scale_color_grey() +
  theme_apa() +
  theme(legend.position = "bottom") +
...
ggplot(gather(data_cleaned_scale[,7:10]), aes(x=value)) + 
  geom_histogram(stat = "count") + facet_wrap(~key, scales = 'free_x') +
  scale_color_grey() +
  theme_apa() +
  theme(legend.position = "bottom") +
...
ggplot(gather(data_cleaned_scale[,31:42]), aes(x=value)) + 
  geom_histogram(stat = "count") + facet_wrap(~key, scales = 'free_x') +
  scale_color_grey() +
  theme_apa() +
  theme(legend.position = "bottom") +
...
for(i in 1:10){
  upper = length(which(nflex_inv[i]==6))/n_row
  lower = length(which(nflex_inv[i]==1))/n_row
...
## Create a list used to delete item that have >.20 extreme values.
list = c()
for(i in 1:10){
...
cortest.bartlett(R = poly_nflex_inv_reduct, n = n_row)
...
# Compute bartlett, KMO and check if we got an identity matrix
cortest.bartlett(R = poly_nflex_inv_reduct_msa, n = n_row)
...
#### *Calcul du R² original de chaque item*
```{r}
# FOnction pour vérifier les conditions d'application de la régression
ConditionRegression <- function(model, df){
  #Installation des packages si non présent
  
  # List of packages for session
  .packages = c("mediation", "jtools", "lme4", "compare", "lindia", "gvlma")
  ...
  #Fonction
  monModel <- model
  # apaT <- apa.reg.table(fit, prop.var.conf.level = 0.95)
  
  
  #Outliers
  gvmodel <- gvlma(monModel)
  plot(gvmodel)
  summary(gvmodel)
  summary(deletion.gvlma(gvmodel))
  plot(deletion.gvlma(gvmodel))
  ggDiagnose <- gg_diagnose(monModel, scale.factor = 1, max.per.page = 1)
  

}

# Détermination des communalités initiales pour l'AFE à partir des R² des régressions multiples
...
pa = fa.parallel(poly_nflex_inv_reduct_msa, fm="pa", fa="fa", main = "Scree Plot", se.bars = T, n.obs = n_row)
...
# bassA = bassAckward(nflex_inv_reduct[,-c(5,6)], nfactors = c(1,1), fm = "pa", cor = "poly")
```
\
...
```{r}
omega(m=nflex_inv_reduct_msa, poly=T, nfactors = 1, fm = "pa", rotation="none", plot = T)
psych::alpha(nflex_inv_reduct_msa)
```


The Omega hierarchical is an estimation of the general factor saturation in a scale. The Omega asymptotic coefficient can be compare to a Guttman $\lambda^6$ (or the Cronbach $\alpha$).
\
\
*Références* 
...
summary(model_cfs, fit.measures=T)


# test avec une afe de la structure
poly = polychoric(data_cleaned_scale[,31:42])
poly = poly$rho

cortest.bartlett(R = poly, n = n_row)
KMO(poly)
det(poly)

pa = fa.parallel(poly, fm="pa", fa="fa", main="Scree Plot", se.bars=T, n.obs=350)
...
pa = fa.parallel(poly, fm="pa", fa="fa", main = "Scree Plot", se.bars = T, n.obs = n_row)
...
```{r}
#Sum for the all scales
data_cleaned_wcst$sum_nflex = apply(data_cleaned_wcst[,c("nflex_2","nflex_3","nflex_5","nflex_7")], 1, sum)
data_cleaned_wcst$sum_as = apply(data_cleaned_wcst[,c(11:20)], 1, sum)
data_cleaned_wcst$sum_cr = apply(data_cleaned_wcst[,c(7:10)], 1, sum)
data_cleaned_wcst$sum_cfs = apply(data_cleaned_wcst[,c(31:42)], 1, sum)
```
...
a = ggplot(data_cleaned_wcst, aes(x=sum_nflex, y=sum_cr))+
  geom_point()+
...
b = ggplot(data_cleaned_wcst, aes(x=sum_nflex, y=sum_as))+
  geom_point()+
  geom_smooth(method="lm", se=TRUE)+geom_jitter() +
  theme_apa() +
  xlab("GCF") +
  ylab("ASQ")

c = ggplot(data_cleaned_wcst, aes(x=sum_nflex, y=sum_cfs))+
  geom_point()+
  geom_smooth(method="lm", se=TRUE)+geom_jitter() +
  theme_apa() +
  xlab("GCF") +
...
d = ggplot(data_cleaned_wcst, aes(x=sum_nflex, y=pourcentage))+
  geom_point()+
  geom_smooth(method="lm", se=TRUE)+geom_jitter() +
  theme_apa() +
  xlab("GCF") +
...
figure = ggarrange(d,c,b,a, ncol=2, nrow=2)
figure

Hmisc::rcorr(as.matrix(data_cleaned_wcst[,c(46, 49:52)]), type="pearson")
```
